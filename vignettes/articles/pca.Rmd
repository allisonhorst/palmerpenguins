...
The penguins data contains several size measurement variables that are correlated. Let's take a look at the correlation matrix with the [corrr package](https://corrr.tidymodels.org/) and the `corrr::correlate()` function:
...
Body mass and flipper length appear highly correlated, but neither of the bill variables appears to be very correlated with any other variables.
...
We'll use the [recipes](https://recipes.tidymodels.org) package from [tidymodels](https://www.tidymodels.org) to do a PCA. First, we'll also use a few recipe steps for preprocessing the data for PCA. To get started with the recipes package, try [this article](https://www.tidymodels.org/start/recipes/).
...
This output is a tidy version of this using `stats::princomp()`:

```{r princomp, eval=TRUE}
penguins %>% 
  dplyr::select(where(is.numeric)) %>% 
  tidyr::drop_na() %>% 
  princomp(cor = TRUE) %>%  # centers and scales
  loadings()
```

Or this code using `stats::prcomp()` instead:
...
Let's do the same thing for principal components 2 and 3. 
